...
from dlhlp_lib.parsers.raw_parsers import LJSpeechRawParser, LJSpeechInstance
...
class LJSpeechPreprocessor(BasePreprocessor):
    ...
    def __init__(self, src: str, root: str) -> None:
        ...
        self.src_parser = LJSpeechRawParser(src)
        self.data_parser = DataParser(root)

    def parse_raw_process(self, instance: LJSpeechInstance) -> None:
        query = {
    ...
    def clean(self):
        cleaned_data_info_path = f"data_config/LJSpeech/clean.json"
    ...
    def split_dataset(self):
        ...
        with open(f"data_config/LJSpeech/config.yaml", 'w') as yamlfile:
            config = {
                "name": "LJSpeech",
                "lang_id": "en",
    ...
    def log(self, msg):
        print(f"[LJSpeechPreprocessor]: ", msg)
