...
from dlhlp_lib.parsers.raw_parsers import CSS10RawParser, CSS10Instance
...
class CSS10Preprocessor(BasePreprocessor):
    ...
    def __init__(self, src: str, root: str) -> None:
        ...
        self.src_parser = CSS10RawParser(src)
        self.data_parser = DataParser(root)
        self.lang = self.src_parser.lang
        self.spk = f"css10-{self.src_parser.lang}"

    def parse_raw_process(self, instance: CSS10Instance) -> None:
        query = {
    ...
    def preprocess(self):
        ...
        phoneset_path = f"{os.path.dirname(__file__)}/../MFA/CSS10/{self.lang}/phoneset.txt"
    ...
    def split_dataset(self):
        ...
        with open(f"data_config/CSS10/{self.lang}/config.yaml", 'w') as yamlfile:
            config = {
                "name": f"CSS10/{self.lang}",
    ...
    def _prepare_mfa_dir(self, mfa_data_dir: Path) -> None:
        ...
        queries = self.data_parser.get_all_queries()
        target_dir = mfa_data_dir / self.spk
    ...
    def mfa(self, output_path) -> None:
        ...
        mfa_dir = f"{os.path.dirname(__file__)}/../MFA/CSS10/{self.lang}"
