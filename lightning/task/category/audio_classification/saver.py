...
class Saver(Callback):
    ...
    def on_train_batch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule, outputs, batch, batch_idx) -> None:
        ...
        if step % trainer.log_every_n_steps == 0 and (batch_idx + 1) % self.config["train_config"]["optimizer"]["grad_acc_step"] == 0:
            ...
            self.log_text(logger, pl_module.mapper.classes[gt_label], step, "Train/GT")
            self.log_text(logger, pl_module.mapper.classes[pred_label], step, "Train/Pred")
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        output = record['output']
        self.val_labels["gt"].extend(output["gt"])
        self.val_labels["pred"].extend(output["pred"])
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        acc = 0
        assert len(self.val_labels["gt"]) == len(self.val_labels["pred"])
        for a, b in zip(self.val_labels["gt"], self.val_labels["pred"]):
            if a == b:
                acc += 1
        acc /= len(self.val_labels["gt"])
        pl_module.log_dict({f"Val/Acc": acc}, sync_dist=True, batch_size=pl_module.bs)
        with open(acc_file_path, 'a') as f:
            f.write(f"Epoch {pl_module.current_epoch}: {acc * 100:.2f}%\n")

        # log classification results
        for i in range(2):
            self.log_text(logger, self.val_labels['gt'][i], step, f"Val/GT-{i}")
            self.log_text(logger, self.val_labels['pred'][i], step, f"Val/Pred-{i}")
