...
class MTLSaver(Callback):
    def __init__(self, config):
        ...
        self.val_loss_dicts = []
        self.val_transcriptions = defaultdict(lambda: defaultdict(list))
    ...
    def on_train_batch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule, outputs, batch, batch_idx) -> None:
        ...
        if step % trainer.log_every_n_steps == 0 and (batch_idx + 1) % self.config["train_config"]["optimizer"]["grad_acc_step"] == 0:
            ...
            beam_search_results = pl_module.mappers[tid].beam_search_decoder(output["emissions"], output["emission_lengths"])
            labels, _ = output["batch"]
            gt_label = labels["texts"][0][:labels["text_lens"][0]].cpu()
            gt_transcript = pl_module.mappers[tid].beam_search_decoder.idxs_to_tokens(gt_label)
            gt_transcript = " ".join([p for p in gt_transcript if p != "|"])

            beams = beam_search_results[0]
            pred_transcript = pl_module.mappers[tid].beam_search_decoder.idxs_to_tokens(beams[0].tokens)
            pred_transcript = " ".join([p for p in pred_transcript if p != "|"])
            
            self.log_text(logger, gt_transcript, step, f"[{tid}] Train/GT")
            self.log_text(logger, pred_transcript, step, f"[{tid}] Train/Pred")
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        output = record['output']
        self.val_transcriptions[tid]["gt"].extend(output["gt"])
        self.val_transcriptions[tid]["pred"].extend(output["pred"])
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        for tid in pl_module.mappers:
            ...
            # log asr results
            for i in range(2):
                self.log_text(logger, self.val_transcriptions[tid]['gt'][i], step, f"[{tid}] Val/GT-{i}")
                self.log_text(logger, self.val_transcriptions[tid]['pred'][i], step, f"[{tid}] Val/Pred-{i}")
